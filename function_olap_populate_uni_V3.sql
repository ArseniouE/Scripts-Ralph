...
-- DROP FUNCTION olapts.populate_olap_uni();
...
IF EXISTS (select from olapts.refreshhistory where tablename = 'ABTEIRESIASDATA') THEN
	...
	insert into olapts.abteiresiasdata
		...
		(case when (teiresiasdata.jsondoc_ ->>'TbScoreDate') like '%ðì' 
			then to_char(to_date(replace(replace((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ðì','AM'),'ìì','PM'),'dd/mm/yyyy'), 'yyyy-mm-dd')
			 when (strpos((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ìì') <> 0 or strpos((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ðì') <> 0)
			 	then to_char(to_date(replace(replace((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ðì','AM'),'ìì','PM'),'dd/mm/yyyy'), 'yyyy-mm-dd')
			 when (strpos((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ìì') = 0 or strpos((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ðì') = 0)
			 	and (strpos((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'AM') = 0 and strpos((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'PM') = 0)
			    then to_char(to_date(replace(replace((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ðì','AM'),'ìì','PM'),'dd/mm/yyyy'), 'yyyy-mm-dd')
				 else 
				 to_char(to_date(replace(replace((teiresiasdata.jsondoc_ ->>'TbScoreDate'),'ðì','AM'),'ìì','PM'),'mm/dd/yyyy'), 'yyyy-mm-dd')
