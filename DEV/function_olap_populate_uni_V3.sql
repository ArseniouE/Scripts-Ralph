...
--IMPORT FOREIGN SCHEMA tenant LIMIT TO (etlloadtime) FROM SERVER matenantserver INTO madata;
...
IF EXISTS (select from olapts.refreshhistory where tablename = 'ABLEVERAGEINDICATION') THEN
	...
	insert into olapts.ableverageindication
		...
		(l3.jsondoc_ ->> 'Data') AS highleveragecustomerval, --added 2025-02-12
		...
		LEFT JOIN madata.custom_lookup as l3 ON l3.t_ = 'LeverageHighCustomer' and l3.jsondoc_->>'Key' = leverageindication.jsondoc_->>'HighLeverageCustomer'
...
IF EXISTS (select from olapts.refreshhistory where tablename = 'ABETLLOADTIME') THEN
raise notice '% - Step abetlloadtime - part a start', clock_timestamp();
insert into olapts.abetlloadtime
	SELECT
		etlloadtime.id_ AS id_,
		etlloadtime.pkid_::varchar as pkid_,
		(etlloadtime.jsondoc_ ->> 'Id') AS etlloadtimeId,
		(etlloadtime.jsondoc_ ->> 'FinishedDate')::timestamp without time zone AS FinishedDate,
		(etlloadtime.jsondoc_ ->> 'ReferenceDate') as ReferenceDate,
		etlloadtime.wfid_::varchar,
		etlloadtime.taskid_::varchar,
		etlloadtime.versionid_::int4,
		etlloadtime.isdeleted_::boolean,
		etlloadtime.islatestversion_::boolean,
		etlloadtime.baseversionid_::int4,
		etlloadtime.contextuserid_::varchar,
		etlloadtime.isvisible_::boolean,
		etlloadtime.isvalid_::boolean,
		etlloadtime.snapshotid_::int4,
		etlloadtime.t_::varchar,
		etlloadtime.createdby_::varchar,
		etlloadtime.createddate_::timestamp,
		etlloadtime.updatedby_::varchar,
		etlloadtime.updateddate_::timestamp,
		CASE WHEN etlloadtime.updateddate_ > etlloadtime.createddate_ THEN etlloadtime.updatedby_ ELSE etlloadtime.createdby_ END AS sourcepopulatedby_,
		GREATEST(etlloadtime.createddate_, etlloadtime.updateddate_) AS sourcepopulateddate_
		,current_setting('myvariables.popdate')::timestamp as populateddate_
	FROM
		madata.etlloadtime
	WHERE	
		GREATEST(etlloadtime.updateddate_, etlloadtime.createddate_) > (select COALESCE(max(asofdate), to_timestamp(0)) from olapts.refreshhistory where tablename = 'ABETLLOADTIME')
		AND GREATEST(etlloadtime.updateddate_, etlloadtime.createddate_)::timestamp <= max_refreshhistory 
		AND etlloadtime.t_ = 'EtlLoadTime'
	;
raise notice '% - Step abetlloadtime - part a end', clock_timestamp();
--------------------------------------------

ELSE
	raise notice '% Step abetlloadtime - part b start', clock_timestamp();
	DROP TABLE IF EXISTS olapts.abetlloadtime;
	CREATE TABLE olapts.abetlloadtime AS
	SELECT
		etlloadtime.id_ AS id_,
		etlloadtime.pkid_::varchar as pkid_,
		(etlloadtime.jsondoc_ ->> 'Id') AS etlloadtimeId,
...
raise notice '% - Step abetlloadtime - part b end', clock_timestamp();

--abetlloadtime
raise notice '% - Step abetlloadtime_idx - part a start', clock_timestamp(); 
DROP INDEX if exists olapts.abetlloadtime_idx;
DROP INDEX if exists olapts.abetlloadtime_idx2;
CREATE INDEX IF NOT EXISTS abetlloadtime_idx ON olapts.abetlloadtime (id_);
CREATE INDEX IF NOT EXISTS abetlloadtime_idx2 ON olapts.abetlloadtime (pkid_,versionid_);

raise notice '% - Step abetlloadtime_idx - part a end', clock_timestamp(); 
END IF;	

------------------------------------------------------------------------

select COALESCE(max(asofdate),to_timestamp(0)) into varprevsuccessdate from olapts.refreshhistory where tablename = 'ABETLLOADTIME';
delete from olapts.refreshhistory where tablename = 'ABETLLOADTIME';
insert into olapts.refreshhistory(tablename,asofdate,prevsuccessdate) select 'ABETLLOADTIME' tablename, max_refreshhistory as  asofdate,varprevsuccessdate;

raise notice '% - Step abetlloadtime - part c end', clock_timestamp();
...
IF EXISTS (select from olapts.refreshhistory where tablename = 'ABOPERATINGRISKASSESSMENT') THEN
	...
	insert into olapts.aboperatingriskassessment
		...
		(operatingriskassessment.jsondoc_->> 'NonTriggers') AS nontriggers,	--added 26/2/2025	
		(SELECT STRING_AGG(t2.jsondoc_->>'Value', ', ') 
		FROM madata.custom_lookup t2 
		WHERE t2.t_='OprOperatingRiskNonTriggers' and not t2.isdeleted_ and t2.jsondoc_->>'Key' IN
			(SELECT jsonb_array_elements_text((operatingriskassessment.jsondoc_->> 'NonTriggers')::jsonb))) AS nontriggersval, --added 26/2/2025	
		(operatingriskassessment.jsondoc_->> 'OperatingRiskFlag') AS operatingriskflag,		
		(l1.jsondoc_->> 'Value') AS operatingriskflagval,		
		(operatingriskassessment.jsondoc_->> 'Triggers') AS triggers,	--added 26/2/2025	
		(SELECT STRING_AGG(t2.jsondoc_->>'Value', ', ') 
		FROM madata.custom_lookup t2 
		WHERE t2.t_='OprOperatingRiskTriggers' and not t2.isdeleted_ and t2.jsondoc_->>'Key' IN
			(SELECT jsonb_array_elements_text((operatingriskassessment.jsondoc_->> 'Triggers')::jsonb))) AS triggersval, --added 26/2/2025			
...
ALTER FUNCTION olapts.populate_olap_uni()
    OWNER TO olap;
